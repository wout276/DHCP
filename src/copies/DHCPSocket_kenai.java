...
public class DHCPSocket extends MulticastSocket  {
    ...
    public DHCPSocket (int inPort) throws SocketException, IOException {
	super(inPort);
        Enumeration<NetworkInterface> nets = NetworkInterface.getNetworkInterfaces();
        for (NetworkInterface netint : Collections.list(nets)) {
            if (netint.isUp() && !netint.isLoopback()) {
                joinGroup(new InetSocketAddress("224.0.0.1", inPort), netint);
            }
        }
