...
public class dhcpclient{
	...
	if (args.length > 0) {
	    hwaddr = args[0];
            ...
            //TODO: ccthien change your MAC here
            hwaddr = "00:19:D2:66:52:47";
	    //System.out.println("Usage: dhcpclient <ethernet_addresss>\n\t ie. dhcpclient 12:34:56:76:0f:08");
	    //System.exit(1);
	}

	try {
            DHCPSocket mySocket = new DHCPSocket(DHCPMessage.CLIENT_PORT); // create socket
...
class Client extends Thread {
    ...
    public void printBytes(byte[]b) {
        for(int i=0;i<b.length;i++) {
            System.out.print(String.format("%2X", b[i])+":");
        }
        System.out.println();
    }
	...
	try {
	    DHCPMessage offerMessageIn = new DHCPMessage(serverIP, portNum);
            ...
            while(b) {
                if (bindSocket.receive(offerMessageIn)) {
                    offerMessageIn.printMessage();
		    printBytes(offerMessageIn.getChaddr());
		    System.out.println(bytesToString(offerMessageIn.getYiaddr()));
                }
            }
		...
		case DHCPMessage.OFFER:
		    printBytes(messageIn.getChaddr());
		...
		case DHCPMessage.ACK:
		    ...
		    t2 = messageIn.getOption(T2_TIME);
                    ...
                    System.out.print(this.getName());
                    System.out.print(" received an DHCPACK and a leasetime.");
                    System.out.print("\nBinding to IP address: ");
                    System.out.print(bytesToString(messageIn.getYiaddr())+"\n");
                    if(t1!=null) {
                        System.out.print("Goodnight for ");
                        //messageIn.printMessage();
	...
	try {
	    ...
	    System.out.println(bytesToString(offerMessageIn.getOption(REQUESTED_IP)));
	    boolean sentinal = true;
	    while (sentinal) {
		if (bindSocket.receive(messageIn)) {
		    if (messageOut.getXid() == messageIn.getXid()) {
